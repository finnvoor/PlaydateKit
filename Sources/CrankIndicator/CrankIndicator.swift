...
public final class CrankIndicator {
    ...
    private func loadImages(for scale: Display.Scale) throws(Playdate.Error) {
        ...
        let imagePath = "CrankIndicator/crank-notice-bubble-\(scale.rawValue)x.png"
        ...
        if let bubbleImage {
            ...
            crankFrames = try Graphics.BitmapTable(path: "CrankIndicator/crank-frames-\(scale.rawValue)x.png")
            frameCount = (crankFrames?.imageCount ?? 0) * 3

            switch scale {
            case .oneTimes, .twoTimes:
                textFrameImage = try Graphics.Bitmap(path: "CrankIndicator/crank-notice-text-\(scale.rawValue)x.png")
                textFrameCount = 14
